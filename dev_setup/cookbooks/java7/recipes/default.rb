...
# FIXME: add other major distro support
  ...
  java7_oracle_path = node[:java7][:oracle_home] + "/bin"
  java7_openjdk_path = node[:java7][:openjdk_home] + "/bin"
  ...
  java7_exec = java7_path + "/java"
  expanded_exec = `which #{java7_exec}`
  unless $? == 0
    Chef::Log.info("An installation of OpenJDK 7 was not found at the required location of #{java7_path}")
    java7_path = java7_oracle_path
    Chef::Log.info("Will now search for Oracle JDK 7 at #{java7_path}")
  ...
  Chef::Log.error("OpenJDK 7 not supported on this platform.")
end
